...
const fallbackCommentary = [
  "The room is warming up. Someone is about to make a bold emotional decision.",
...
export default function DemoAuctionPage() {
  ...
  useEffect(() => {
    ...
    const auctionChannel = supabase
      .channel("bragwall-mobile-auction-commentary")
      .on(
        "postgres_changes",
        {
          event: "*",
    ...
    const bidsChannel = supabase
      .channel("bragwall-mobile-bids-commentary")
      .on(
        "postgres_changes",
        {
          event: "INSERT",
  ...
  async function placeBid(increment: number) {
    ...
    const commentary =
      amount >= 1000
        ? `${bidderName} just pushed this masterpiece to R${amount.toLocaleString()}. Serious bragging rights are now in play.`
        : `${bidderName} enters the race at R${amount.toLocaleString()}. The auction is officially heating up.`;
  ...
  const commentary =
    auction.mc_commentary || fallbackCommentary[commentIndex];
