...
export default function AdminLivePage() {
  ...
  const queuedArtworks = artworks.filter((artwork) => artwork.status !== "sold");
  ...
  useEffect(() => {
    ...
    const auctionChannel = supabase
      .channel("admin-live-auction-state-premium-final")
      .on(
        "postgres_changes",
        {
          event: "*",
    ...
    const artworksChannel = supabase
      .channel("admin-live-artworks-premium-final")
      .on(
        "postgres_changes",
        {
          event: "*",
    ...
    const bidsChannel = supabase
      .channel("admin-live-bids-premium-final")
      .on(
        "postgres_changes",
        {
          event: "*",
    ...
    const activityChannel = supabase
      .channel("admin-live-activity-premium-final")
      .on(
        "postgres_changes",
        {
          event: "*",
            ...
            <section className="h-full overflow-y-auto bragwall-live-scroll">
              <header className="px-6 pt-6 pb-5">
                <div className="flex items-start justify-between gap-8">
                  ...
                  <div className="grid grid-cols-3 gap-3.5 min-w-[520px] pt-5">
                    <TopMetric
                      label="Status"
                    ...
                    <section className="rounded-[30px] border border-white/10 bg-white/[0.045] p-4.5 shadow-[0_35px_100px_rgba(0,0,0,0.38)]">
                      <div className="grid grid-cols-[1fr_270px] gap-4 items-start">
                        ...
                        <div className="rounded-[28px] bg-[#061124]/95 border border-white/12 p-4 shadow-2xl flex flex-col">
                          ...
                          <div className="mt-3.5 rounded-[22px] bg-white text-[#07152b] p-4 shadow-xl">
  <p className="uppercase tracking-[0.28em] text-[8px] text-slate-400 font-black mb-2.5">
    AI Auction MC
  </p>
                ...
                <div className="mt-4.5 rounded-[22px] border border-white/10 bg-white/[0.04] px-5 py-3.5 flex items-center justify-between shadow-xl">
                  <div className="flex items-center gap-3">
                    ...
                    <p className="text-white/65 font-bold">
                      Tip: Upload artworks in Artwork Upload to build your queue
                      before going live.
                    </p>
                  </div>
...
function SidebarLink({
  ...
  return (
    ...
    >
      <span className={`h-[22px] w-[22px] flex items-center justify-center ${toneText(tone)}`}>
        {icon}
      </span>
      <span>{label}</span>
    </a>
