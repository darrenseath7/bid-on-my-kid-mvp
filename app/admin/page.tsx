...
export default function AdminDashboardPage() {
  ...
  return (
    <main className="min-h-screen overflow-hidden bg-[#020b18] text-white">
      <div className="fixed inset-0 pointer-events-none bg-[radial-gradient(circle_at_16%_12%,rgba(22,214,109,0.18),transparent_30%),radial-gradient(circle_at_88%_12%,rgba(255,200,87,0.12),transparent_30%),linear-gradient(180deg,#061124,#020b18_56%,#010712)]" />
      <div className="fixed inset-0 pointer-events-none opacity-[0.06] bg-[linear-gradient(rgba(255,255,255,0.13)_1px,transparent_1px),linear-gradient(90deg,rgba(255,255,255,0.13)_1px,transparent_1px)] bg-[size:44px_44px]" />

      <section className="relative min-h-screen p-2.5 md:p-3.5">
        <div className="min-h-[calc(100vh-20px)] overflow-hidden rounded-[24px] border border-white/10 bg-[#061124]/70 shadow-[0_30px_140px_rgba(0,0,0,0.76)] md:rounded-[30px]">
          <div className="grid min-h-[calc(100vh-20px)] lg:grid-cols-[248px_1fr]">
            <AdminSidebar />

            <section className="min-w-0 overflow-y-auto px-4 py-5 md:px-7 md:py-7">
              <div className="mb-5 flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
                <div>
                  <p className="mb-2 text-[10px] font-black uppercase tracking-[0.32em] text-[#16d66d]">
                    BragWall Dashboard
                  </p>
                  <h1 className="text-3xl font-black tracking-[-0.05em] md:text-5xl">
                    Welcome back.
                  </h1>
                  <p className="mt-2 max-w-3xl text-sm font-semibold leading-relaxed text-white/58 md:text-base">
                    Here is what is happening with {summary.schoolName || "your auction"}.
                  </p>
                </div>
              ...
              {errorMessage && (
                <div className="mb-5 rounded-[22px] border border-[#ffc857]/25 bg-[#ffc857]/10 px-5 py-4 text-sm font-bold leading-relaxed text-[#ffc857]">
                  {errorMessage}
                </div>
              )}
              ...
              <div className="grid gap-5 xl:grid-cols-[1.15fr_0.85fr]">
                <section className="rounded-[28px] border border-white/10 bg-[#07162b]/88 p-5 shadow-2xl md:rounded-[34px] md:p-6">
                  <div className="mb-5 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                    <div>
                      <h2 className="text-2xl font-black tracking-[-0.04em] md:text-3xl">Getting started</h2>
                      <p className="mt-1 text-sm font-semibold text-white/52">Complete these steps to run a smooth BragWall auction.</p>
                    </div>

                    <div className="min-w-[220px]">
                      <div className="mb-2 flex items-center justify-between text-[11px] font-black uppercase tracking-[0.18em] text-white/50">
                  ...
                  <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
                    ...
                    <StepCard complete={summary.unsoldArtworksCount > 0 || summary.soldArtworksCount > 0} title="Prepare live room" text="Test the event controls before parents start bidding." action="Go To Live Room" href="/admin/live" />
                ...
                <section className="relative overflow-hidden rounded-[28px] border border-[#16d66d]/20 bg-[#07162b]/88 p-5 shadow-2xl md:rounded-[34px] md:p-6">
                  <div className="absolute right-[-130px] top-[-130px] h-80 w-80 rounded-full bg-[#16d66d]/12 blur-3xl" />
                  <div className="absolute inset-0 bg-[url('/paintbrush.jpg')] bg-cover bg-center opacity-[0.10]" />
                  <div className="absolute inset-0 bg-[#07162b]/80" />
                  <div className="relative">
                    <p className="mb-3 text-[10px] font-black uppercase tracking-[0.32em] text-[#16d66d]">Auction total</p>
                    <h2 className="text-[52px] font-black leading-none tracking-[-0.08em] text-[#16d66d] md:text-[68px]">{formatCurrency(summary.totalRaised)}</h2>
                    ...
                    <a href="/admin/sales" className="mt-5 block rounded-2xl bg-[#16d66d] px-5 py-3.5 text-center text-sm font-black text-[#031124] shadow-[0_18px_45px_rgba(22,214,109,0.24)] transition hover:scale-[1.01]">
                      Open Sales Records
                    </a>
                  </div>
                </section>
              ...
              <div className="mt-5 grid gap-5 xl:grid-cols-[0.95fr_1.05fr]">
                <section className="rounded-[28px] border border-white/10 bg-[#07162b]/88 p-5 shadow-2xl md:rounded-[34px] md:p-6">
                  <h2 className="text-2xl font-black tracking-[-0.04em]">Recent activity</h2>
                ...
                <section className="rounded-[28px] border border-white/10 bg-[#07162b]/88 p-5 shadow-2xl md:rounded-[34px] md:p-6">
                  <h2 className="text-2xl font-black tracking-[-0.04em]">Top performing schools</h2>
                  <p className="mt-1 text-sm font-semibold text-white/45">By total sales raised across BragWall.</p>
                  <div className="mt-4 space-y-3">
...
function AdminSidebar() {
  return (
    <aside className="hidden border-r border-white/10 bg-[#061124]/92 p-4 lg:flex lg:min-h-[calc(100vh-20px)] lg:flex-col">
      <div className="mb-6 rounded-[22px] px-2 py-3">
        <img src="/bragwall-logo.png" alt="BragWall" className="h-16 w-auto" />
        <p className="mt-2 text-[8px] font-black uppercase tracking-[0.34em] text-white/42">Young Art - Big Pride</p>
      </div>
      ...
      <div className="relative mt-auto overflow-hidden rounded-[22px] border border-white/10 bg-white/[0.045] p-4">
        <div className="absolute inset-0 bg-[url('/paintbrush.jpg')] bg-cover bg-center opacity-20" />
        <div className="absolute inset-0 bg-[#061124]/78" />
        <div className="relative">
      ...
      <div className="mt-3 rounded-[22px] border border-white/10 bg-white/[0.045] p-3">
        <AdminLogoutButton />
      </div>
    </aside>
  );
...
function SidebarLink({ href, label, icon, active = false }: { href: string; label: string; icon: ReactNode; active?: boolean }) {
  return (
    <a href={href} className={`flex items-center gap-3 rounded-[14px] border px-3.5 py-3 text-sm font-bold transition ${active ? "border-[#16d66d]/50 bg-[#16d66d]/22 text-white shadow-[0_0_26px_rgba(22,214,109,0.18)]" : "border-transparent text-white/70 hover:border-white/10 hover:bg-white/[0.06] hover:text-white"}`}>
...
function MetricCard({ label, value, subtext, icon, tone, highlight = false }: { label: string; value: string; subtext: string; icon: ReactNode; tone: "green" | "yellow" | "purple"; highlight?: boolean }) {
  return (
    <div className={`rounded-[22px] border p-4 shadow-2xl ${highlight ? "border-[#16d66d]/28 bg-[#16d66d]/10" : "border-white/10 bg-white/[0.055]"}`}>
      <div className="flex items-center gap-4">
        <div className={`flex h-14 w-14 items-center justify-center rounded-[18px] border ${toneClasses(tone).box}`}>{icon}</div>
        <div className="min-w-0">
          <p className="text-[10px] font-black uppercase tracking-[0.24em] text-white/42">{label}</p>
          <p className="mt-1 truncate text-3xl font-black tracking-[-0.05em]">{value}</p>
          <p className="mt-1 text-xs font-semibold text-white/45">{subtext}</p>
        </div>
      </div>
    </div>
  );
...
function StepCard({ complete, title, text, action, href }: { complete: boolean; title: string; text: string; action: string; href: string }) {
  return (
    <div className="rounded-[22px] border border-white/10 bg-[#061124]/84 p-4">
      ...
      <p className="mt-2 min-h-[48px] text-center text-xs font-semibold leading-relaxed text-white/50">{text}</p>
